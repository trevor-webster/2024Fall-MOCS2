...
begin 
	# import libraries. 
	# No more than one call, we need to wrap the cell 
	using Plots
	using PlutoUI
end
...
What you'll find in our collection of Pluto notebook for modeling complex systems:

 - `chaos.jl`: cobweb plot, bifurcation
 - `lotka-volterra.jl`: phase diagram plot using makie.jl
 - `rosetta-models.jl`: Rosetta models, implemented in different ways (timestepping, modelling toolkit, vanilla OrdinaryDiffEq, computational simulations, ...). Right
    
	- Exponential growth
	- Logistic growth (SI model): more than one box
	- Birth-Death process: open population
	- SIR 
	- Lotka-Volterra
	- Romeo and Juliet
	- Pendulum: phase space representation, units, higher-order derivatives
	- Lorenz attractors: chaos

Julia is great for scientific computing. In this notebook, I do a whirlwind tour of Julia's ecosystem for scientific computing. To do so, we take advantage of Pluto's reactivity, or how cells update based on other cells.
...
# ╔═╡ 1db45776-9cac-4785-8c7e-4815e7e44a03
?plot # see Live Docs

# ╔═╡ 44762cdb-f778-4540-b455-5eb8ba523b8c
plotattr("linestyle") # then we can query plotattr to get more details
