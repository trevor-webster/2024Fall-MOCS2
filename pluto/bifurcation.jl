...
# ╔═╡ af252265-7ff4-4c14-bfdc-e5fc0fa584ba
begin
...
# ╔═╡ ca460406-7cfe-11ef-1b40-7fae9eff0077
md"# Bifurcation"

# ╔═╡ 880d177e-7448-437c-9635-f28338d75931
let
	
	f = Figure(size=(800,400))
	ax = Axis(f[1,1], ylabel=L"f(x) = r - x^2", xlabel="x")
	
	myf(x; r=1) = r - x^2
	lines!(ax, -2:0.1:2, x -> myf(x), label="r=2", color=:blue)
	lines!(ax, -2:0.1:2, x -> myf(x, r=0.1), label="r=0.1", color=:orange)
	lines!(ax, -2:0.1:2, x -> myf(x, r=1.5), label="r=1.5", color=:green)
	axislegend()
	
	ax = Axis(f[1,2], ylabel="X'", xlabel="X")
	lines!(ax, -2:0.01:2, x -> x * (2 - x^2), linestyle=:dash, color=:blue) 
	lines!(ax, -2:0.01:2, x -> x * (0.1 - x^2), linestyle=:dash, color=:orange) 
	lines!(ax, -2:0.01:2, x -> x * (1.5 - x^2), linestyle=:dash, color=:green) 
	f
end

# ╔═╡ 0f5ac9b9-563a-4d6e-929a-29cc0b116b77
let
	r=1.0	
	f = Figure()
	ax = Axis(f[1,1])
	scatter!(ax, 0:0.01:1, x -> x * (r - x^2), 
				 label="N(t+1) = f(N(t))",
			  	 color=:red) 
	f
end
